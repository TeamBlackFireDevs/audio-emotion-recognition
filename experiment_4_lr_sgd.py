...
# Experiment 4: Linear Regression with Stochastic Gradient Descent (SGD)
# For a real-valued regression task on audio features (demo purpose)
...
# -----------------------------
# Utility functions adapted for regression target
# -----------------------------
def parse_cremad_label(filename):
    parts = filename.split('.')[0].split('_')
    if len(parts) >= 3:
        code = parts[2].upper()
        # Map emotion to a numeric score for demo regression task
        mapping = {
...
class LinearRegressionSGD:
    ...
    def fit(self, X, y):
        ...
        for epoch in range(self.max_epochs):
            ...
            self_losses.append(epoch_loss)
            if (epoch+1) % 100 == 0:
                print(f"Epoch {epoch+1} - MSE: {epoch_loss:.4f}")
        self.losses = self_losses
...
def plot_losses(losses):
    ...
    plt.title('Linear Regression with SGD Learning Curve')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('experiment_4_lr_sgd_loss.png', dpi=300, bbox_inches='tight')
    plt.show()


def main():
    print("=== Experiment 4: Linear Regression with SGD ===")
    ...
    model = LinearRegressionSGD(learning_rate=0.01, max_epochs=1000)
    ...
    with open('experiment_4_lr_sgd_results.txt', 'w') as f:
        f.write("=== Experiment 4: Linear Regression with SGD Results ===")
